...
def find_references(graph: Graph, include_classes: bool = True) -> Tuple[Set[str], Dict[str, str], Dict[str, str]]:
    ...
    """
    module_refs = set()
    class_info = {}  # class URI -> label/comment
    property_info = {}  # property URI -> label/comment
    ...
    if include_classes:
        # Find classes with their labels and comments
        ...
        # Find properties with their labels and comments
        for s in graph.subjects(RDF.type, OWL.ObjectProperty):
            if isinstance(s, URIRef):
                ...
                property_info[uri] = info
                
    return module_refs, class_info, property_info

def infer_module_relationships(module: str, class_info: Dict[str, Dict], all_modules: Set[str]) -> Tuple[Set[str], str]:
    ...
    # Look for semantic relationships in class names and comments
...
def analyze_module_content(module: str, class_info: Dict[str, Dict], property_info: Dict[str, Dict]) -> str:
...
def main():
    ...
    # Find all module references from main guidance
    referenced, main_classes, main_properties = find_references(main_graph)
    logger.info(f"References from guidance.ttl: {referenced}")
    
    # Load all modules and find cross-references
    modules_dir = Path('guidance/modules')
    all_modules = {f.stem for f in modules_dir.glob('*.ttl')}
    
    # Track module relationships and content
    ...
    for module in all_modules:
        try:
            ...
            # Store explicit references and content
            module_refs[module] = refs
            module_classes[module] = classes
            module_properties[module] = properties
            
            # Add inferred relationships
            inferred, reason = infer_module_relationships(module, classes, all_modules)
            if inferred:
