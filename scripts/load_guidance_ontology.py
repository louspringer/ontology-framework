...
class GraphDBClient:
    ...
    def create_repository(self, repo_id: str, title: str) -> bool:
        ...
        # Create repository using SYSTEM repository
        url = f"{self.base_url}/repositories/SYSTEM/statements"
        ...
        try:
            response = self.session.post(url, headers=headers, data=config)
    ...
    def load_ontology(self, file_path: Path, repo_id: str) -> bool:
        ...
        try:
            # Read and parse the file with RDFlib to handle base URI
            g = Graph()
            # Set base URI to the file's directory for relative imports
            base_uri = f"file://{file_path.parent.absolute()}/"
            g.parse(str(file_path), format="turtle", publicID=base_uri)
            
            # Convert to string with proper base URI
...
def get_guidance_modules() -> List[Path]:
    ...
    # Get all other .ttl files except core files
    other_files = [
...
def main() -> None:
    ...
    # Configuration
    repo_id = "test-ontology-framework"
    ...
    for module in modules:
        if not client.load_ontology(module, repo_id):
            ...
            sys.exit(1)
        # Wait between loads to ensure proper ordering
        time.sleep(1)
