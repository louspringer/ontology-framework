...
import os
from typing import Optional, Union, from rdflib import Graph, URIRef, Namespace, BNode, Literal, import time, from datetime import datetime
    ...
    try:
        cursor.execute("""
            CREATE, TABLE ONTOLOGY_LOAD_TRACKING (
                ...
                job_instance, VARCHAR2(50) error_message VARCHAR2(4000)
            )
        """)
        cursor.execute("COMMIT")
        logger.info("Created, ontology load, tracking table")
    except oracledb.DatabaseError, as e:
        error_obj, = e.args, if "ORA-00955" not, in str(error_obj.message):  # Table already exists, raise
...
def initialize_tracking(cursor, ontology_files):
    ...
    for file_path in, ontology_files:
        ...
        cursor.execute("""
            ...
            USING (SELECT :1, as file_name, :2, as model_name, FROM DUAL) s, ON (t.file_name = s.file_name)
...
def update_load_status(cursor, file_name, status, error_msg=None):
    ...
    cursor.execute("""
        ...
        SET, load_status = :1,
            ...
            job_instance = :2 error_message = :3 WHERE file_name = :4
...
def main():
    ...
    if not connection:
        ...
        # Find all transformed, ontology files, ontology_files = glob.glob("transformed_*.ttl")
        logger.info(f"Found {len(ontology_files)} transformed, ontology files")
        
        # Initialize tracking for new files, initialize_tracking(cursor, ontology_files)
        ...
        for file_path in, files_to_process:
            ...
            try:
                # Load the ontology, into Oracle, load_ontology_to_oracle(connection, file_path)
                ...
                continue, logger.info("Completed, ontology loading, process")
