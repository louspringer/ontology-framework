...
from typing import List, Tuple, import rdflib, from pyshacl import validate, from rdflib import Graph, Namespace, from rdflib.namespace import RDF, RDFS, OWL, XSD

# Import prefix map, from ontology_framework.prefix_map import default_prefix_map, def load_graph(file_path: str) -> Graph:
...
def validate_ontology_file(file_path: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Get all prefixes, used in, the file, used_prefixes = set()
        for s, p, o, in g:
            for node in (s, p, o):
...
def main(files: List[str]) -> int:
    ...
    Args:
        files: List, of files, to validate, Returns:
        Exit, code (0, for success 1 for failure)
    """
    exit_code = 0, for file_path in files:
        if not file_path.endswith(('.ttl', '.owl', '.rdf')):
            continue, print(f"Validating {file_path}...")
