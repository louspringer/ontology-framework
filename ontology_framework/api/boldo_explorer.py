...
from ..exceptions import BoldoAPIError, AuthenticationError
...
class BoldoAPIExplorer:
    ...
    def _get_api_key_from_1password(self) -> str:
        ...
        try:
            ...
            if api_key := os.getenv("BOLDO_API_KEY"):
                return api_key
            
            # Then try 1Password CLI if available
            result = subprocess.run(
                ["op", "read", "op://Private/Boldo API Key/credential"],
                capture_output=True,
                text=True
            )
            ...
            raise ValueError("Failed to get API key from 1Password")
        except (subprocess.SubprocessError, ValueError) as e:
            self.logger.error(f"Failed to get API key: {e}")
            raise AuthenticationError("Failed to get API key") from e
