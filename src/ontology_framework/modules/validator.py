...
class ValidationTarget:
    ...
    def __str__(self) -> str:
        return f"ValidationTarget(uri={self.uri}, type={self.target_type}, priority={self.priority}, errors={self.errors})"
...
""")
    ...
    def validate_shacl(self, graph: Graph) -> List[str]:
        ...
        try:
            ...
            shapes_graph = Graph()
            # Load SHACL shapes from external file
            shapes_file = Path(__file__).parent.parent.parent / 'guidance_shapes.ttl'
...
SELECT ?component ?quality WHERE {
    ?component a :ValidationRule ;
        :hasQuality ?quality .
    FILTER(?quality < 0.85)
}
