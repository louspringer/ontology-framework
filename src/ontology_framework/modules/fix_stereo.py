...
def fix_stereo_ttl(input_file: str, output_file: str) -> None:
    """
    Fix issues in stereo.ttl file:
    1. Fix prefix usage to be consistent
    2. Fix frequency specification to use string type
    3. Add proper SHACL shapes
    """
    # Create a new graph
    g = Graph()
    ...
    # Fix frequency specifications
    for s, p, o in g.triples((None, EX.estimatedCost, None)):
    ...
    freq_shape = EX.FrequencySpecificationShape
    g.add((freq_shape, RDF.type, SH.NodeShape))
    g.add((freq_shape, SH.targetClass, EX.FrequencySpecification))
    
    # Add property shape for frequency
    prop_shape = EX.FrequencyPropertyShape
    ...
    active_shape = EX.ActiveCrossoverSetupShape
    g.add((active_shape, RDF.type, SH.NodeShape))
    g.add((active_shape, SH.targetClass, EX.ActiveCrossoverSetup))
    
    # Add property shape for frequency specification
    freq_spec_shape = EX.FrequencySpecificationPropertyShape
    g.add((freq_spec_shape, RDF.type, SH.PropertyShape))
