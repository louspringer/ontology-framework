...
def initialize_tools(config: Dict[str, Any]) -> tuple[MCPCore, BFG9KTargeter, MaintenanceServer]:
    ...
    return core, targeter, server

def recreate_ontology(
    ontology_path: Path,
    core: MCPCore,
    targeter: BFG9KTargeter,
    server: MaintenanceServer
) -> ValidationResult:
    ...
    try:
        ...
        # Execute recreation phase
        logger.info(f"Executing recreation phase for {ontology_path}")
        ...
        server.update_validation(validation_id, "completed")
