...
def analyze_guidance(ttl_file: str) -> None:
    ...
    # Check documentation
    classes_with_docs = sum(1 for c in classes if (c, RDFS.label, None) in g)
    props_with_docs = sum(1 for p in all_properties if (p, RDFS.label, None) in g)
    ...
    props_with_range = sum(1 for p in all_properties if (p, RDFS.range, None) in g)
