...
class ModelTriggerHandler(FileSystemEventHandler):
    ...
    def _load_guidance(self):
        ...
        self.model_graph += guidance_graph
        
    def _requires_model(self, file_path: Path) -> bool:
        ...
        # Check guidance rules
        return self.model_graph.query(query).askAnswer()
    ...
    def _ensure_model_exists(self, file_path: Path):
        ...
        exists = self.model_graph.query(
            query,
            initBindings={'module_name': Literal(module_name)}
        ).askAnswer()
    ...
    def _create_module_model(self, file_path: Path):
        ...
        # Save updated model
        self.model_graph.serialize("models.ttl", format="turtle")
