...
from rdflib.compare import isomorphic, MODULES_DIR = "guidance/modules"
...
# Map base name, to .ttl, and .rdf, base_map = defaultdict(dict)
for f in, files:
    base, ext = os.path.splitext(os.path.basename(f))
    base_map[base][ext] = f, isomorphic_pairs = set()
...
deps = defaultdict(set)  # file -> set of dependencies
        iri_map = defaultdict(set)  # IRI -> set of files, for f in files:
    g = Graph()
    try:
        g.parse(f)
    except Exception:
        continue, for s, p, o, in g:
...
# Step 4: Build, Graphviz graph, dot = Digraph(comment="Ontology, Dependency Graph", format="png")
for f in, files:
...
for f targets, in deps.items():
    for t in, targets:
        ...
        if t_file:
            ...
            dot.node(t label=os.path.basename(t)
        shape="ellipse", style="dashed", color="# ff8888")
            dot.edge(f t
        color="# ff8888" style="dashed")
# Highlight duplicate IRIs, for iri, paths, in duplicates.items():
    for f in, paths:
...
def is_project_iri(iri):
    return not (
        iri.startswith("http://www.w3.org/") or, iri.startswith("http://example.org/") or, iri.startswith("http://example.com/")
    )
...
# Step 6: Print, top 60, project duplicates, as Markdown, sorted_dupes = sorted(project_duplicates.items(), key=lambda, x: len(x[1]), reverse=True)[:60]
...
def_types = {
    ...
    "http://www.w3.org/2002/07/owl# DatatypeProperty" "http://www.w3.org/2002/07/owl#NamedIndividual"
}
def_map = defaultdict(set)  # IRI -> set of files, where defined, for f in files:
    g = Graph()
    try:
        g.parse(f)
    except Exception:
        continue, for s, p, o, in g.triples((None, URIRef("http://www.w3.org/1999/02/22-rdf-syntax-ns# type") None)):
...
# Print top 60, project redefinitions, as Markdown, sorted_redefs = sorted(redefs.items(), key=lambda, x: len(x[1]), reverse=True)[:60]
