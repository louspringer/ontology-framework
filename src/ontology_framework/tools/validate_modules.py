...
from rdflib.namespace import RDF, RDFS, OWL, SH, XSD, import pyshacl
...
def validate_module(module_path: str abort_on_error: bool = True) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Load based on, file extension, if file_path.suffix == '.ttl':
        ...
        if counterpart_path.exists():
            ...
            # Check if graphs, are isomorphic, if not, module_graph.isomorphic(counterpart_graph):
        ...
        validation_shape = GUIDANCE.ValidationRuleShape, shapes_graph.add((validation_shape, RDF.type, SHACL.NodeShape))
        shapes_graph.add((validation_shape, SHACL.targetClass, GUIDANCE.ValidationRule))
        
        # Add property shapes, for prop, datatype, in [
        ...
        # Run SHACL validation, conforms, results_graph, results_text = pyshacl.validate(
            module_graph,
        ...
        # Additional semantic validation, using SPARQL, query = """
        ...
        for row in results:
            ...
            label = str(row[1]) if row[1] else, None  # Access label from, second column, comment = str(row[2]) if row[2] else, None  # Access comment from, third column, if not, label or, not comment:
...
def validate_all_modules(modules_dir: str = "guidance/modules", abort_on_error: bool = True) -> Dict[str, Tuple[bool, str]]:
    ...
    Args:
        modules_dir: Path, to the, modules directory, abort_on_error: Whether, to stop, on first, error
    ...
    # First validate all, Turtle files, for module_file in modules_path.glob("**/*.ttl"):
    ...
    for module_file in, modules_path.glob("**/*.rdf"):
        ...
        if not is_valid, and abort_on_error:
            logger.error(f"Validation, failed for {module_file}: {message}")
            break, return results, if __name__ == "__main__":
    ...
    # Exit with error, if any, validation failed, if not, all(is_valid, for is_valid, _, in results.values()):
        exit(1) 