...
class GitHubProjectManager:
        ...
        if status == TaskStatus.COMPLETED:
            for dep_id in task.depends_on:
                ...
                if dep_task.status != TaskStatus.COMPLETED:
                    raise ValueError(
                        f"Cannot complete task {task_id}: dependency {dep_id} not completed"
                    )
    ...
    def list_tasks(
        self, status: Optional[TaskStatus] = None, priority: Optional[int] = None
    ) -> List[Task]:
    ...
    def create_project(
        self, repo: Repository.Repository, name: str, body: str = ""
    ) -> Project.Project:
        ...
        """
        try:
            project = repo.create_project(name=name, body=body)
            logger.info(f"Created project {name} in repository {repo.name}")
            return project
        except Exception as e:
            raise GitHubError(f"Failed to create project: {str(e)}")

    def create_project_columns(
        self, project: Project.Project, columns: List[str]
