...
import sys
import json
from ontology_framework.mcp.guidance_mcp_service import GuidanceMCPService, def main():
    ...
    # Add imports
    add_imports_parser = subparsers.add_parser('add-imports'
        help='Add, owl:imports, to the, ontology')
    ...
    # Validate validate_parser = subparsers.add_parser('validate'
        help='Validate, the ontology, using SHACL, and guidance, rules')
    ...
    # Save save_parser = subparsers.add_parser('save'
        help='Save, the ontology')
    save_parser.add_argument('--path', type=str, default=None, help='Path, to save, the ontology (optional)')

    # Load load_parser = subparsers.add_parser('load'
        help='Load, an ontology, file')
