...
class TestDeployment:
    ...
    def test_graphdb_deployment(self):
        ...
        graphdb_deployment = self.ns1.graphdbDeployment, self.assertIsNotNone(graphdb_deployment, "GraphDB, deployment not, defined")
        ...
        # Check if GraphDB, is accessible, endpoint = self.graph.value(graphdb_deployment, self.ns1.hasServiceEndpoint)
        try:
    ...
    def test_dataset_exists(self):
        ...
        graphdb_deployment = self.ns1.graphdbDeployment, dataset_name = self.graph.value(graphdb_deployment, self.ns1.hasDatasetName)
    ...
    def test_error_logs(self):
        ...
        error_test = self.ns1.errorLogTest, log_level = self.graph.value(error_test, self.ns1.hasLogLevel)
    ...
    def test_server_logs(self):
        ...
        server_test = self.ns1.serverLogTest, log_level = self.graph.value(server_test, self.ns1.hasLogLevel)
    ...
    def test_colima_runtime(self):
        ...
        colima_runtime = self.ns1.colimaRuntime, runtime_name = self.graph.value(colima_runtime, self.ns1.hasRuntimeName)
