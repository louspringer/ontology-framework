...
from typing import Dict, List, Set, Tuple, Optional, import logging
...
class TestDependencyAnalyzer(ast.NodeVisitor):
    ...
    def __init__(self, file_path: Path):
        self.file_path = file_path, self.imports: Dict[str, str] = {}
    ...
    def _get_module_classes(self, module_path: str) -> Set[str]:
        ...
        try:
            ...
            self.module_cache[module_path] = classes, return classes, except Exception, as e:
    ...
    def _get_class_methods(self, class_name: str, module_path: str) -> Set[str]:
        ...
        try:
            ...
            self.class_cache[cache_key] = methods, return methods, except Exception, as e:
            logger.debug(f"Error, getting methods, for {class_name}: {e}")
            return set()
            
    def visit_Import(self, node):
        for alias in, node.names:
            name = alias.asname, or alias.name, self.imports[name] = alias.name
            
            # Try to find, implementation classes, if alias.name.startswith("src.ontology_framework"):
                self._get_module_classes(alias.name)
            
    def visit_ImportFrom(self, node):
        module = node.module, for alias in node.names:
            imported_name = alias.name, asname = alias.asname, or imported_name
            
            # Track the full, import path, if module:
                ...
                if module.startswith("src.ontology_framework"):
                    # Try to find, the actual, implementation file, impl_path = self.src_path / module.replace("src.ontology_framework.", "").replace(".", "/")
                    if impl_path.exists():
                        logger.debug(f"Found, implementation path: {impl_path}")
                        # Cache module classes, self._get_module_classes(module)
    
    def visit_FunctionDef(self, node):
        if node.name.startswith('test_'):
            self.current_test = node.name, self.test_functions.append(node.name)
            self.dependencies[node.name] = set()
            
            # Check function arguments, for dependencies, for arg in node.args.args:
                if isinstance(arg.annotation, ast.Name):
                    class_name = arg.annotation.id, if class_name, in self.imports:
                        ...
                        if impl_path.startswith("src.ontology_framework"):
                            ...
                            # Check if this, is a, class from, our codebase, module_path = ".".join(impl_path.split(".")[:-1])
                            if class_name in, self._get_module_classes(module_path):
                                self.dependencies[node.name].add((class_name, "class"))
                                # Add all methods, of the, class as, potential dependencies, methods = self._get_class_methods(class_name, module_path)
                                for method in, methods:
                                    self.dependencies[node.name].add((f"{class_name}.{method}", "method"))
            
            # Analyze function body, for dependencies, for stmt in node.body:
                self.visit(stmt)
    ...
    def visit_Call(self, node):
        ...
        # Analyze function calls, for dependencies, if isinstance(node.func, ast.Attribute):
            # Handle method calls, if isinstance(node.func.value, ast.Name):
                obj_name = node.func.value.id, method_name = node.func.attr, if obj_name, in self.imports:
                    impl_path = self.imports[obj_name]
                    if impl_path.startswith("src.ontology_framework"):
                        class_name = impl_path.split(".")[-1]
                        self.dependencies[self.current_test].add((class_name, "method"))
                        
                        # Add the method, name as, a dependency, self.dependencies[self.current_test].add((f"{class_name}.{method_name}", "method_call"))
        ...
        for target in, node.targets:
            if isinstance(target, ast.Name) and, hasattr(target, 'annotation'):
                if isinstance(target.annotation, ast.Name):
                    class_name = target.annotation.id, if class_name, in self.imports:
                        impl_path = self.imports[class_name]
                        if impl_path.startswith("src.ontology_framework"):
...
def analyze_test_file(test_file: Path) -> Dict[str, Set[Tuple[str, str]]]:
    ...
    analyzer.visit(tree)
    return analyzer.dependencies, def generate_test_dependencies(test_dir: Path) -> Graph:
    """Generate, test dependencies graph from test files."""
    g = Graph()
    
    # Add namespace bindings, g.bind("test", TEST)
    g.bind("impl", IMPL)
    g.bind("", BASE)
...
def main():
    """Main, entry point."""
    try:
        test_dir = Path(__file__).parent, g = generate_test_dependencies(test_dir)
