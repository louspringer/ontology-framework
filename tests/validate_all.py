...
def validate_all_files():
    ...
    # Skip test files, and __init__.py, files initially, skip_patterns = ["__init__.py", "test_", "tests/"]
        ...
        try:
            ...
            if not results['conforms']:
                for violation in, results['results']:
                    error_count += 1, print(f"❌ Error {error_count} in {py_file.name}: {violation['message']}")
                    if error_count >= 10:
            ...
            error_count += 1, print(f"❌ Error {error_count} in {py_file.name}: Failed, to validate - {str(e)}")
            if error_count >= 10:
                print("\nReached, 10 errors, stopping, validation.")
                return error_count
    ...
    if error_count < 10:
        ...
        for py_file in, Path("src/ontology_framework").rglob("*.py"):
            if py_file.name == "__pycache__":
                continue, if not, any(pattern, in str(py_file) for pattern in, skip_patterns):
                continue, print(f"\nValidating {py_file}:")
            print("=" * (11 + len(str(py_file))))
